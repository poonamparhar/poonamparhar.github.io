---
layout: post
title: "Understanding G1 GC Logs"
date: 2012-06-18
description: 
img:  g1_gclogs.jpg 
...
The purpose of this post is to explain the meaning of various GC log entries generated using a few tracing and diagnostic options for G1 GC. We will take a look at the output generated with **PrintGCDetails**, which is a product flag and provides the most detailed level of information. Along with that, we will  take a look at the output of two diagnostic flags that can be enabled using -XX:+UnlockDiagnosticVMOptions option: **G1PrintRegionLivenessInfo** which prints the occupancy and the amount of space used by live objects in each region at the end of a marking cycle, and **G1PrintHeapRegions** that provides detailed information on the heap regions during their allocation and reclaimation.

We will be going over the logs generated with the JDK 1.7.0_04. Let's start with **PrintGCDetails**.

## -XX:+PrintGCDetails

Here's an example log of G1 collection event generated with the PrintGCDetails option.
...
This is a typical log of an Evacuation Pause (G1 collection), in which live objects are copied from one set of regions (young OR young+old) to another set. It is a stop-the-world activity, and all the application threads are stopped at a safepoint during this time. This pause is made up of several sub-tasks indicated by the indentation of the log entries. Let's look at each of these entries in detail.

```
0.522: [GC pause (young), 0.15877971 secs]
```

This is the highest level information telling us that it is an Evacuation Pause that started after 0.522 secs from the start of the process in which all the regions being evacuated are **young** i.e. Eden and Survivor regions. This collection took 0.15877971 secs to complete.

Evacuation Pauses can be **mixed** as well. In that case, the selected set of regions include all of the young regions as well as a few old regions.
...
Update RS gives us the time each thread spent in updating the Remembered Sets. Remembered Sets are the data structures that keep track of the references pointing into a heap region. Mutator threads keep changing the object graph and thus the references that point into a particular region. We keep track of these changes in buffers called *Update Buffers*. The Update RS sub-task processes the update buffers that could not be processed concurrently, and updates the corresponding remembered sets for all regions.
...
These are the times that each worker thread had spent in scanning the Remembered Sets. Remembered Set of a region contains cards that correspond to the references pointing into that region. This phase scans those cards looking for the references pointing into all the regions of the collection set.
...
These are the times spent by each worker thread copying live objects from the regions in the Collection Set to other regions.
...
Termination time is the time spent by worker threads offering to terminate. But before termination, a worker thread checks the work queues of other threads, and if there still are object references in other work queues then it tries to steal those references. If it succeeds in stealing any references, it processes them and offers to terminate again.
...
These are the lifetimes of all the worker threads.
...
These are the times that each worker thread spent in performing other tasks that have not been accounted for above.

```
[Clear CT: 0.1 ms]
```

This is the time spent in clearing the Card Table. This task is performed in a serial mode.

```
[Other: 1.5 ms]
```

Time spent in a  few other tasks that are listed below. The following sub-tasks (which individually may be parallelized) are
performed serially.
...
This line provides details on the heap size changes after the Evacuation Pause. It shows that Eden had the occupancy of 12M and its capacity
was also 12M before the collection. After the collection, its occupancy got reduced to 0 since everything is evacuated/promoted
from Eden during a collection, and its target size grew to 13M. The new Eden capacity of 13M is not reserved at this point. This value is
the target size of the Eden. Regions are added to Eden as the demand is made and when the added regions reach to the target size, we start
the next collection.

Similarly, Survivors had the occupancy of 0 bytes and it grew to 2048K after the collection. The total heap occupancy and capacity was 14M and 64M
receptively before the collection and it became 9739K and 64M after the collection.
...
The first phase of a marking cycle is Initial Marking where all the objects directly reachable from the roots are marked, and this phase
...
This marks the start of a concurrent phase that scans the set of root-regions which are directly reachable from the survivors of the initial marking
phase.
...
Now, let's look at the output generated with the flag **G1PrintRegionLivenessInfo**. This is a diagnostic option and gets enabled with -XX:+UnlockDiagnosticVMOptions. G1PrintRegionLivenessInfo prints the live data information of each region during the **Cleanup phase** of the concurrent-marking
cycle.
...
Cleanup phase of the concurrent-marking cycle started at 26.896 secs from the start of the process, and this live data information got printed after the marking phase. Committed G1 heap ranges from 0x02e00000 to 0x0fe00000, and the total G1 heap reserved by JVM is from 0x02e00000 to 0x12e00000 address. Each region in the G1 heap is of 1048576 bytes.
...
These are a continuous set of regions called Humongous regions for storing a large object. HUMS (Humongous starts) marks the start of the set of humongous regions, and HUMC (Humongous continues) tags the subsequent regions of the humongous regions set.
...
G1PrintHeapRegions option logs the events pertaining to regions when regions are committed, allocated into, or are reclaimed.
...
This shows the start and end of an Evacuation pause. This event is followed by a GC counter tracking both the evacuation pauses and Full GCs. In this case, it is the 7th GC since the start of the process.
...
Shows the start and end of a Full GC. This event is also followed by the same GC counter as above. This record marks the 17th GC since the start of the process.
...
The region having bottom at 0x6e800000 just started being used for allocation. In this case, it is an Eden region and is being allocated into by a mutator thread.
...
Regions being used for the allocation of an Humongous object. The object spans over two regions.

```
G1HR ALLOC(SingleH) 0x6f900000 0x6f9eb010
```

Single region being used for the allocation of an Humongous object.
...
Here, the allocation request for a Humongous object could not be satisfied with the available free committed regions causing the expansion of the heap. Thus, new regions are committed here, and then allocated into for the Humongous object.
...
Region being used for copying surviving objects into during a GC.
...
## When are Heap Regions reclaimed?
